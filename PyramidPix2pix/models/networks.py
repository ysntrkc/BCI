...
def frequency_division(src_img):
    ...
    image_low = torch.irfft(
        fft_low, signal_ndim=2, onesided=False, signal_sizes=[imgH, imgW]
    )
    image_high = torch.irfft(
        fft_high, signal_ndim=2, onesided=False, signal_sizes=[imgH, imgW]
    )
...
class Attention_UnetGenerator(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_downs - 5):  # add intermediate layers with ngf * 8 filters
            unet_block = Attention_UnetSkipConnectionBlock(
                ngf * 8,
                ngf * 8,
                input_nc=None,
                submodule=unet_block,
